                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 cursor-pointer">
                    <div className="flex flex-col items-start justify-center gap-5 bg-white rounded-lg shadow-lg p-6 w-11/12 md:w-2/3 lg:w-[60vw] lg:h-[90vh] py-10 overflow-y-auto">
                        ...
                        {modalDetails.map((detail, index) => (
                            <div key={index} className={` w-full items-center justify-center flex flex-col gap-5`}>
                                <h4 className={`flex flex-col items-center gap-2 font-semibold font-sora text-sm` }> <span>Question: </span> : {detail.question}</h4>
                                <p className={`flex flex-col items-center gap-2 font-light font-nunito text-sm` } ><span>Answer: </span>{detail.answer}</p>
                            </div>
                        ))}
                        <button
